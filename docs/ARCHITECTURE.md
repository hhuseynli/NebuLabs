...
└────────────┬────────────────────────────┘
    ...
    │                 │
┌───▼──────┐    ┌────▼──────────────┐
│ In-Memory│    │ Groq Chat API     │
│ Storage  │    │ (llama-3.3-70b)   │
│ (local)  │    └───────────────────┘
└──────────┘
```

**Optional Production Setup (see HOSTING_AND_INFRA.md):**

Swap in-memory store for Supabase; deploy frontend to Vercel and backend to Railway.
...
│   └── fundraiser.py
├── services/
│   ├── groq_service.py                # Groq API client (active)
│   ├── gemini_service.py              # Unused (for reference)
