...
export default function ProfilePage() {
  ...
  useEffect(() => {
    api.getProfile(username).then(setProfile).catch(() => setProfile(null));
  }, [username]);
              ...
              <section className="rounded-2xl border border-slate-200 bg-white p-4">
                ...
                <ul className="mt-3 space-y-2 text-sm">
                  {profile.recent_posts.map((post) => (
                    <li key={post.id}><Link className="text-ember-700" to={`/r/${post.community_slug || post.community_id}/post/${post.id}`}>{post.title}</Link></li>
                  ))}
                </ul>
              </section>
              <section className="rounded-2xl border border-slate-200 bg-white p-4">
